...
def render_schema_to_markdown(schema, required_fields=None, depth=0):
    ...
    # Безопасно извлекаем свойства схемы, проверяя их наличие
    properties = getattr(schema, 'properties', None)
    if properties:
        for prop_name, prop in properties.items():
            # Безопасное определение типа (если свойства type нет, пишем object)
            p_type_obj = getattr(prop, 'type', None)
            p_type = p_type_obj.value if hasattr(p_type_obj, 'value') else str(p_type_obj or 'object')
            ...
            if p_type == 'array' and hasattr(prop, 'items') and prop.items:
                items_type_obj = getattr(prop.items, 'type', None)
                items_type = items_type_obj.value if hasattr(items_type_obj, 'value') else str(items_type_obj or 'object')
                p_type = f"array of {items_type}"
            
            lines.append(f"| {indent}`{prop_name}` | {p_type} | {p_req} | {p_desc} |")
            
            # Рекурсивный уход в глубину для сложных объектов
...
def openapi_to_markdown(file_path):
    ...
    # ИСПРАВЛЕНИЕ: Обходим пути. У объекта Path реальный адрес эндпоинта лежит в свойстве .url
    for path_data in specification.paths:
        path_url = getattr(path_data, 'url', None) or getattr(path_data, 'path', '/unknown')
        
        if hasattr(path_data, 'operations') and path_data.operations:
            for operation in path_data.operations:
                method_obj = getattr(operation, 'method', None)
                method = method_obj.value.upper() if hasattr(method_obj, 'value') else str(method_obj or 'GET').upper()
                ...
                if operation.parameters:
                    ...
                    for param in operation.parameters:
                        p_name = param.name
                        p_in = param.location.value if hasattr(param.location, 'value') else str(param.location or "query")
                        
                        p_type = "string"
                        if hasattr(param, 'schema') and param.schema:
                            p_type_obj = getattr(param.schema, 'type', None)
                            p_type = p_type_obj.value if hasattr(p_type_obj, 'value') else str(p_type_obj or "string")
                ...
                if operation.request_body and operation.request_body.content:
                    ...
                    for content in operation.request_body.content:
                        if 'json' in getattr(content, 'media_type', '') and content.schema:
